...
import { BulkAssessmentStatusDto } from '../../common/dto/bulk-assessment-status.dto';
...
export class AssessmentsController {
  ...
  @Post('bulk-delete')
  @Permissions('assessments.delete')
  @ApiOperation({ summary: 'Delete assessments in bulk' })
  async bulkDelete(@Body() payload: BulkDeleteDto, @CurrentUser() actor: CurrentUserContext): Promise<{ deletedCount: number }> {
    return this.assessmentsService.bulkDelete(payload.ids, actor);
  }

  @Post('bulk-status')
  @Permissions('assessments.update')
  @ApiOperation({ summary: 'Update assessments in bulk' })
  async bulkStatus(
    @Body() payload: BulkAssessmentStatusDto,
    @CurrentUser() actor: CurrentUserContext,
  ): Promise<{ updatedCount: number }> {
    return this.assessmentsService.bulkUpdateStatus(payload.ids, payload.status, actor);
  }
}
