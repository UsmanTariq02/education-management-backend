...
export class BatchesController {
  ...
  @Post('bulk-delete')
  @Permissions('batches.delete')
  @ApiOperation({ summary: 'Delete batches in bulk' })
  async bulkDelete(@Body() payload: BulkDeleteDto, @CurrentUser() actor: CurrentUserContext): Promise<{ deletedCount: number }> {
    return this.batchesService.bulkDelete(payload.ids, actor);
  }

  @Post('bulk-status')
  @Permissions('batches.update')
  @ApiOperation({ summary: 'Update batches in bulk' })
  async bulkStatus(
    @Body() payload: BulkUpdateStatusDto,
    @CurrentUser() actor: CurrentUserContext,
  ): Promise<{ updatedCount: number }> {
    return this.batchesService.bulkUpdateStatus(payload.ids, payload.isActive, actor);
  }
}
