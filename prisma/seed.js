...
const organization_module_enum_1 = require("../src/common/enums/organization-module.enum");
const prisma = new client_1.PrismaClient();
const seededModules = organization_module_enum_1.DEFAULT_ORGANIZATION_MODULES;
const seededModuleCount = organization_module_enum_1.DEFAULT_ORGANIZATION_MODULES.length;
...
async function resetDemoOrganizationData(organizationId) {
...
async function seedDemoOrganization(params) {
    ...
    ].map((student) => prisma.student.create({
        data: {
            organizationId: organization.id,
            ...student,
    ...
    const feePlans = await Promise.all(students.map((student) => prisma.feePlan.create({
        data: {
            organizationId: organization.id,
            studentId: student.id,
            batchId: batch.id,
        ...
        data: [
            {
                ...
                channel: 'WHATSAPP',
                target: 'GUARDIAN',
                body: 'Assalam o Alaikum {{guardianName}}, {{studentName}} has a pending fee at {{organizationName}}. Total fee: {{totalFee}}, paid: {{paidFee}}, pending: {{pendingFee}}, cycle: {{billingCycle}}.',
                isActive: true,
            },
            {
                organizationId: organization.id,
                ...
                body: 'Dear {{guardianName}}, {{organizationName}} confirms payment for {{studentName}}. Paid fee: {{paidFee}}, pending fee: {{pendingFee}}.',
                isActive: true,
            },
        ],
    });
...
function buildNorthfieldStudentBlueprints() {
    const firstNames = [
        'Ayaan',
        'Fatima',
        'Zain',
...
async function seedNorthfieldLearningHub(params) {
    ...
    const users = await Promise.all(userBlueprints.map((user) => prisma.user.create({
        data: {
            organizationId: organization.id,
    ...
    const subjects = await Promise.all(subjectBlueprints.map((subject) => prisma.subject.create({
        data: {
            organizationId: organization.id,
            name: subject.name,
            code: subject.code,
    ...
    const teachers = await Promise.all(teacherBlueprints.map((teacher) => prisma.teacher.create({
        data: {
            organizationId: organization.id,
    ...
    await prisma.batchSubjectAssignment.createMany({
        data: batches.flatMap((batch) => subjectBlueprints.map((subject) => ({
            organizationId: organization.id,
    ...
    for (const entry of timetableBlueprints) {
        ...
        const created = await prisma.timetableEntry.create({
            data: {
                ...
                onlineMeetingUrl: entry.deliveryMode === 'ONLINE'
    ...
    const feePlanByStudentId = new Map();
    ...
    await prisma.attendance.createMany({
        data: students.flatMap((student, index) => attendanceDates.map((attendanceDate, dateIndex) => {
            const batchId = studentBatchLinks[index].batchId;
    ...
    const reminderTemplates = await Promise.all([
        prisma.reminderTemplate.create({
            data: {
                ...
                channel: 'WHATSAPP',
                target: 'GUARDIAN',
                body: 'Assalam o Alaikum {{guardianName}}, {{studentName}} has an overdue fee at {{organizationName}}. Total fee: {{totalFee}}, paid: {{paidFee}}, pending: {{pendingFee}}, due date: {{dueDate}}.',
                isActive: true,
            },
        }),
        prisma.reminderTemplate.create({
            data: {
                ...
                target: 'STUDENT',
                subject: 'Payment received for {{studentName}}',
                body: 'Dear {{guardianName}}, {{organizationName}} confirms fee receipt for {{studentName}}. Paid fee: {{paidFee}}, pending fee: {{pendingFee}}.',
                isActive: true,
            },
        }),
    ]);
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
        ...
        const examSubjects = await Promise.all(subjects.map((subject) => prisma.examSubject.create({
            data: {
                examId: exam.id,
                subjectId: subject.id,
                totalMarks: 100,
        ...
        for (let studentIndex = 0; studentIndex < batchStudents.length; studentIndex += 1) {
            ...
            for (const question of objectiveAssessment.questions) {
                if (question.type === 'MCQ' || question.type === 'TRUE_FALSE') {
                    ...
                    const selectedOption = studentIndex % 5 === 0
...
async function main() {
    ...
    const staffPermissionNames = permissionRecords
        ...
        ['academic-sessions.read', 'subjects.read', 'teachers.read', 'organization-assets.read'].includes(permission.name))
        .map((permission) => permission.name);
    ...
    const overdueWhatsappTemplate = await prisma.reminderTemplate.upsert({
        ...
        update: {
            ...
            target: 'GUARDIAN',
            body: 'Assalam o Alaikum {{guardianName}}, this is a fee reminder from {{organizationName}} for {{studentName}}. Billing cycle: {{billingCycle}}. Total fee: {{totalFee}}. Paid fee: {{paidFee}}. Pending fee: {{pendingFee}}. Due date: {{dueDate}}. Please contact the school administration if payment has already been made or if you need any clarification.',
            isActive: true,
        },
        create: {
            organizationId: defaultOrganization.id,
    ...
    const overdueEmailTemplate = await prisma.reminderTemplate.upsert({
        ...
        update: {
            ...
            body: 'Dear {{guardianName}}, this is a fee reminder from {{organizationName}} for {{studentName}}. Billing cycle: {{billingCycle}}. Total fee: {{totalFee}}. Paid fee: {{paidFee}}. Pending fee: {{pendingFee}}. Due date: {{dueDate}}. Please contact the school administration if payment has already been made or if you need any clarification.',
            isActive: true,
        },
        create: {
            organizationId: defaultOrganization.id,
    ...
    const paymentReceiptTemplate = await prisma.reminderTemplate.upsert({
        ...
        update: {
            ...
            body: 'Dear {{guardianName}}, this is a payment confirmation from {{organizationName}} for {{studentName}}. Billing cycle: {{billingCycle}}. Total fee: {{totalFee}}. Paid fee received: {{paidFee}}. Pending fee: {{pendingFee}}. Thank you for your cooperation.',
            isActive: true,
        },
        create: {
            organizationId: defaultOrganization.id,
